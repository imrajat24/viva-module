...
const getByCourseId = async (courseId) => {
  ...
  try {
    const vivasFromDB = await Viva.find(filter)
      .populate("questionPaper")
      .populate("answerSheet")
      .lean();
    if (vivasFromDB.length == 0)
      return createResponse(DB_ERROR_NOT_FOUND, null);
    const vivas = [];
...
const getByCourseIdTraineeId = async (courseId, traineeId) => {
  ...
  try {
    const vivaFromDB = await Viva.findOne(filter)
      .populate("questionPaper")
      .populate("answerSheet")
      .lean();
...
const write = async (viva) => {
  ...
  try {
    const oldVivaFromDB = await Viva.findOne(filter)
      .populate("questionPaper")
      .populate("answerSheet")
      .lean();
